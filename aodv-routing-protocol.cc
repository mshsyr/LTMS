...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////MSH
	.AddAttribute("maliciousAttribute", "An attribute to set up malicious activity",
					UintegerValue(0),
					MakeUintegerAccessor(&RoutingProtocol::SetMalicious,
										&RoutingProtocol::GetMalicious),
					MakeUintegerChecker<uint16_t>())
	.AddAttribute("onOffAttackAttribute", "An attribute to set the period of on off attack",
					UintegerValue(0),
					MakeUintegerAccessor(&RoutingProtocol::SetOnOffAttackPeriod,
										&RoutingProtocol::GetOnOffAttackPeriod),
					MakeUintegerChecker<uint16_t>())
	.AddAttribute("forwardedAttribute", "An attribute to check if the sent packet is forwarded or not",
					Ipv4AddressValue("1.1.1.1"),
					MakeIpv4AddressAccessor(&RoutingProtocol::SetIsForwarded,
										&RoutingProtocol::GetIsForwarded),
					MakeIpv4AddressChecker())
	.AddAttribute("PromiscAttribute", "An attribute to enable the implementation of method PromiscReceive",
					BooleanValue(false),
					MakeBooleanAccessor(&RoutingProtocol::SetPromiscImp,
										&RoutingProtocol::GetPromiscImp),
					MakeBooleanChecker ())
	.AddAttribute("optionAttribute", "An attribute to set up malicious activity option",
					UintegerValue(0),
					MakeUintegerAccessor(&RoutingProtocol::SetOption,
										&RoutingProtocol::GetOption),
					MakeUintegerChecker<uint16_t>())
	.AddAttribute("onOffPercentAttribute", "An attribute to set the percentage of the on phase to the off phase",
					DoubleValue(0),
					MakeDoubleAccessor(&RoutingProtocol::SetOnOffPercent,
										&RoutingProtocol::GetOnOffPercent),
					MakeDoubleChecker<double>())
	.AddAttribute("initPeriodAttribute", "An attribute to set the initialization period",
					UintegerValue(0),
					MakeUintegerAccessor(&RoutingProtocol::SetInitPeriod,
										&RoutingProtocol::GetInitPeriod),
					MakeUintegerChecker<uint16_t>())
...
RoutingProtocol::Start ()
{
  ...
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////MSH: 
  // To register SetPromiscReceiveCallback on netDevice, Next, we have to write the implementation of method PromiscReceive
  if (GetPromiscImp ()) //Writing the implementation enable the promiscous mode for all nodes; so, an attribute is defined to specify where to enable it
  {
	m_ipv4->GetNetDevice (1)->SetPromiscReceiveCallback (MakeCallback (&RoutingProtocol::PromiscReceive, this));
  }
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////MSH
  if	(GetMalicious()) 
  {
		// First scenario when the node is developig good behaviour
		if (GetMalicious()== 1)
		{
			if(ns3::Simulator::Now().GetSeconds ()>1) // This first second of simulation is for initializaton
			{
				...
				if((temp%36)+ns3::Simulator::Now().GetSeconds ()-2<18)
				{
					NS_LOG_ERROR("D "<<ns3::Simulator::Now().GetSeconds()<<" Source: "<<header.GetSource ()<<" Destination: "<<header.GetDestination ());
					//std::cout <<"D "<< ns3::Simulator::Now().GetSeconds ()<<" "<<header.GetSource ()<<" " <<header.GetDestination ()<<" " <<"malicious activity"<<std::endl;
					return false;
				}
			}
		}
//*********************************************************************************************************************************************************************************
		else if(GetMalicious()== 2) // Second scenario when the node is developig bad behaviour
		{
			if(ns3::Simulator::Now().GetSeconds ()>1) // This first second of simulation is for initializaton
			{
				Ptr<UniformRandomVariable> x = CreateObject<UniformRandomVariable> ();
...
//*********************************************************************************************************************************************************************************
		else if(GetMalicious()== 3) // Third scenario when the node change its behaviour from good to bad suddenly
		{
			if(ns3::Simulator::Now().GetSeconds ()>1) // This first second of simulation is for initializaton
			{
				Ptr<UniformRandomVariable> x = CreateObject<UniformRandomVariable> ();
				...
				x->SetAttribute("Max", DoubleValue(300));
				uint16_t temp = x->GetInteger ();
				if (ns3::Simulator::Now().GetSeconds ()<20) // Less than 20 seconds, the node is behaving good
				{
					if((temp%36)+ns3::Simulator::Now().GetSeconds ()-2<18)
					{
						NS_LOG_ERROR("D "<<ns3::Simulator::Now().GetSeconds()<<" Source: "<<header.GetSource ()<<" Destination: "<<header.GetDestination ());
...
//*********************************************************************************************************************************************************************************
		else if (GetMalicious()== 4 && GetOnOffAttackPeriod()) // The fourth scenario the fixed on off attack ( the period of the on and off phases are preset)
		{
			if(ns3::Simulator::Now().GetSeconds ()>1) // This first second of simulation is for initializaton
			{
				Ptr<UniformRandomVariable> x = CreateObject<UniformRandomVariable> ();
				...
				x->SetAttribute("Max", DoubleValue(300));
				uint16_t temp = x->GetInteger ();
				if (ns3::Simulator::Now().GetSeconds ()<GetInitPeriod()) // Less than the initialization period, the node is behaving good
				{
					//if((temp%36)+ns3::Simulator::Now().GetSeconds ()-2<GetInitPeriod()-2)
					if (temp%10<1)
					{
						NS_LOG_ERROR("D "<<ns3::Simulator::Now().GetSeconds()<<" Source: "<<header.GetSource ()<<" Destination: "<<header.GetDestination ());
						//std::cout <<"D "<< ns3::Simulator::Now().GetSeconds ()<<" "<<header.GetSource ()<<" " <<header.GetDestination ()<<" " <<"malicious activity"<<std::endl;
				...
				else // The node after the initial period starts its on off attack
				{
					uint16_t onOffPeriod = GetOnOffAttackPeriod();
					uint16_t loc = (ns3::Simulator::Now().GetSeconds ()-GetInitPeriod())/onOffPeriod; //a variable to specify where we are located in on of off period
					...
					if (loc%2==0)
					{
						if (temp2%4<3)  // Change the malicious drop rate default (temp2%4<3)
						{
							NS_LOG_ERROR("D "<<ns3::Simulator::Now().GetSeconds()<<" Source: "<<header.GetSource ()<<" Destination: "<<header.GetDestination ());
							return false;
						}
					...
					else 
					{
						if (temp2%10<1)
						{
							//NS_LOG_ERROR("D AODV Malicious: "<<ns3::Simulator::Now().GetSeconds()<<" Source: "<<header.GetSource ()<<" Destination: "<<header.GetDestination ());
							NS_LOG_ERROR("D "<<ns3::Simulator::Now().GetSeconds()<<" Source: "<<header.GetSource ()<<" Destination: "<<header.GetDestination ());
							return false;
						}
					}
...
//*********************************************************************************************************************************************************************************	
		else if (GetMalicious()== 5 && GetOnOffAttackPeriod()) // The fifth scenario the percentage on off attack ( the percentage between on and off phases are preset)
		{
			if(ns3::Simulator::Now().GetSeconds ()>1) // This first second of simulation is for initialization
			{
				Ptr<UniformRandomVariable> x = CreateObject<UniformRandomVariable> ();
				x->SetAttribute ("Min", DoubleValue (0));
				x->SetAttribute("Max", DoubleValue(300));
				uint16_t temp = x->GetInteger ();
				//std::cout<<"The temp variable is: "<<temp<<std::endl;
				if (ns3::Simulator::Now().GetSeconds ()<GetInitPeriod()) // Less than the initialization period, the node is behaving good
				{
					//if((temp%36)+ns3::Simulator::Now().GetSeconds ()-2<GetInitPeriod()-2)
					if (temp%10<1)
				...
				else // The node after the initial period seconds starts its on off attack
				{
					uint16_t offPeriod = GetOnOffAttackPeriod();
					float onPeriod =  offPeriod*GetOnOffPercent();
					uint16_t totalPeriod = onPeriod + offPeriod;
					std::cout<<"The totalPeriod value is: "<<totalPeriod<<std::endl;
					uint16_t loc = ((ns3::Simulator::Now().GetSeconds ()-20)/totalPeriod)+1; //a variable to specify where we are located in on or off period
					std::cout<<"The loc value is: "<<loc<<std::endl;
					Ptr<UniformRandomVariable> z = CreateObject<UniformRandomVariable> ();
					z->SetAttribute ("Min", DoubleValue (0));
					z->SetAttribute ("Max", DoubleValue(300));
					uint16_t temp2 = z->GetInteger ();
					if (ns3::Simulator::Now().GetSeconds () <= (20+loc*totalPeriod-offPeriod) )// if it is true then we are in the on phase
					{
						if (temp2%4<3)  //temp2%4<3  //temp2%10<8
						{
							NS_LOG_ERROR("D "<<ns3::Simulator::Now().GetSeconds()<<" Source: "<<header.GetSource ()<<" Destination: "<<header.GetDestination ());
							return false;
						}
...
bool RoutingProtocol::PromiscReceive (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol, const Address &from,
                                 const Address &to, NetDevice::PacketType packetType)
{
	...
	//////////////////////////////////////////Call the Trust Forwarded Input Method
